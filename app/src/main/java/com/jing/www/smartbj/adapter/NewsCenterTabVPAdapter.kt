...
class NewsCenterTabVPAdapter(//通过构造传入数据 //设置适配器,该ViewPAger要存储Views,beanList数据
        var views: List<NewsCenterContentTabPager?>?, private val tabBeenList: List<NewsCenterBean.NewsCenterNewsTabBean?>?) : PagerAdapter() {
    ...
    override fun getPageTitle(position: Int): CharSequence? {
        return tabBeenList!![position]?.title
    }
}